...
the measures, while the boxes represent the first and last quartile,
...
When failures are not endemic
events, \ie as soon as the node MTBF is at least 15 years
...
The next contender, \fifofixed, requires a quarter less bandwidth
...
benefits from not blocking when the filesystem is not available to checkpoint,
which is sufficient, when failures are rare, to obtain a significant
performance gain. % 4.9TB/s at 24 years
All Daly-based strategies benefit from their lesser I/O pressure, and reach the
target efficiency with around half the bandwidth needed by \propfixed. The
highlight these results call attention upon is that C/R strategies can scale to
satisfy the need of future platforms, either by integrating I/O-aware scheduling
strategies in the software infrastructure of prospective systems or by heavily
investing in over-provisioning their practical I/O bandwidth.
