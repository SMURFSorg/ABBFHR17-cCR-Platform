...
the checkpointing policy, the I/O workloads, the interferences that the
...
are all scheduled using the same algorithm wether they
...
happen at date $\period{i}-\ckpt{i}$ (and the first checkpoint is
...
the appropriate checkpointing period can be challenging. The observed
duration of checkpoints varies depending on how much interference happen,
on average. Similarly, when employing I/O scheduling algorithms,
...
introduced delays) swell the checkpoint duration to $C_{swollen}$, the effective
period differs from the desired period by the difference $C_{swollen}-\ckpt{i}$.
...
not at $t_{req}$, which is another improvement when compared to
\fifoblock and \nocoop.
