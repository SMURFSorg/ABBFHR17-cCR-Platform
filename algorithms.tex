...
The previous strategy trades the cost of I/O interferences for for idle time, as jobs
perform a blocking (idle) wait for the I/O token.  If the application developer can
refactor the program code to continue computing while awaiting I/O request
completions, it becomes possible to replace otherwise idle wait time with useful
computation. In the \fifononblock algorithm, when the previous checkpoint ends at
time $t_{now}$, a tentative time for the next checkpoint is set at
$t_{req}=t_{now}+\period{i}-\ckpt{i}$.  At time $t_{req}$, a non-blocking I/O request
is made to request the I/O token -- the I/O token is still scheduled FCFS according
to request arrival time.  The job continues its computation until the scheduler
informs the job that the I/O token is available. At this point, the job must
immediately (or after a short synchronization) generate its checkpoint
data\footnote{In user-level checkpointing the job typically finishes its current
  computing block before generating its checkpoint data.}.  In this work, we assume
that this resynchronization cost is negligible relative to the checkpoint commit
duration.
%
\dca{I commented out the example libraries because the described app-to-library
  ...
  needed in this case}
% Popular checkpointing libraries, like SCR~\cite{Moody10_SCR} or
% FTI~\cite{Bautista-Gomez11_FTI}, already support such functionality through API
% calls to probe whether a checkpoint should be taken at that time or not.  
Indeed, postponing checkpoint I/O increases a job's exposure to failures.  However,
...
at $t_{req}$ -- a fact that may mitigate the increased risk exposure of \fifoblock
versus \nocoop algorithms.
% , which is another improvement when compared to the \fifoblock and \nocoop
% algorithms.

 
% Then, the application initiates its I/O (checkpointing, initial input, final
% output or recovery). When the active application completes its I/O, the next
% requesting application (in FCFS order) become the active I/O application.


% GB: The variants are described in the Simulation
% Again, we consider two variants in the \fifononblock algorithm where checkpoints
% are tentatively taken at 1) fixed frequency (\fifofixed), or 2) at the Daly
% frequency (\fifodaly).
...
\dca{lots of references to ``application'' instead of ``job'' here ...}
...
\begin{compactitem}
\item Category \IOcat $\Catiocat$: Applications $A_{i}$, $1\leq i \leq r$ with an
  (input, output or recovery) I/O request of length $v_{i}$ seconds and enrolls $q_{i}$
  processors. $A_{i}$ initiated its I/O request $d_{i}$ seconds ago and has been idle
  for $d_{i}$ seconds.
...
\item Category \Ckptcat $\Catckptcat$: Applications $A_{i}$, $r+1\leq i \leq r+s$,
  ...
  $A_{i}$ took its last checkpoint $d_{i}$ seconds ago and keeps executing until the
  I/O token is availalbe for a new checkpoint. Since $A_{i}$ is a candidate,
  $d_{i} \geq \period{Daly}(A_{i})$
\end{compactitem}
