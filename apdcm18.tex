...
To illustrate the second point, we conducted the following simulation (whose
parameters and conditions are described in more
details in Section~\ref{sec:results}). Consider a workload consisting of many
different applications that uses C/R as their fault-tolerance
method. Applications either apply the fixed checkpoint period of one hour,
or the optimal period give by the Young/Daly Formula. When two or more
checkpoints overlap, we consider two scenarios: either
they are slowed down proportionally to the amount of processes that
share the bandwidth (we call this \emph{No Interference Management},
or the first checkpoint completes before the next one can start (we
call this \emph{FCFS Interference Management}, and discuss further how
this can be implemented in Section~\ref{sec:feasibility}).
...
also by changing the optimal checkpoint interval), 
applications spend more time to commit their checkpoint 
than they would if there was no
interference. Figures~\ref{fig:ckps-slow-160}
and~\ref{fig:ckps-slow-40} illustrate how much the average checkpoint
commit is slowed down as a function of the machine MTBF, for two
system I/O bandwidth: 160GB/s, which is the theoretical peak of the
machine, and 40GB/s, which represents a degraded but realistic value of the achievable
bandwidth when there is no interference.

\begin{figure}[t]
  \begin{center}
    \includegraphics[width=.9\linewidth]{sim/figures/ckpt-slowdown-160Gbs.pdf}
    \caption{\label{fig:ckps-slow-160}Slowdown of Checkpoints due to
      Interference for the APEX Workshop workflow when simulating the
      Cielo platform with an effective bandwidth of 160GB/s.}
  \end{center}
\end{figure}

\begin{figure}[t]
  \begin{center}
    \includegraphics[width=.9\linewidth]{sim/figures/ckpt-slowdown-40Gbs.pdf}
    \caption{\label{fig:ckps-slow-40}Slowdown of Checkpoints due to
      Interference for the APEX Workshop workflow when simulation the
      Cielo platform with an effective bandwidth is 40GB/s.}
  \end{center}
  \end{figure}

When the system bandwidth is at its peak
(Figure~\ref{fig:ckps-slow-160}), the Young/Daly formula provides a
critical tool to minimize interference; however, when no cooperation
between the applications is enforced, a significant interference is
observed, and applications spend in average twice the time in
checkpointing that they did expect. A simple interference management
policy, like FCFS, reduces this dramatically, for all values of the
MTBF.  However, when the bandwidth is more constraint
(Figure~\ref{fig:ckps-slow-40}), and when the node MTBF is shorter,
interferences introduce a significant slowdown for all
strategies. This is consistent with practical observations of
interference conducted on various HPC systems~\cite{Yildiz2016,MubarakCJLJSRCB17}.
