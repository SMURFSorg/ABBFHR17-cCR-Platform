...
intensive non-checkpoint I/O and others may write very large checkpoints.\\
  ...
  % I/O activities.
$\bullet$ \emph{CR-CR I/O contention}: Most importantly, there is a high
...
checkpoint commit time, efficiency will decrease.}.

\begin{figure*}[t]
  \begin{center}
    \includegraphics[width=.4\linewidth]{sim/figures/ckpt-slowdown-160Gbs.pdf} \quad
     \includegraphics[width=.4\linewidth]{sim/figures/ckpt-slowdown-40Gbs.pdf}
 \end{center}
    \caption{\label{fig:ckps-slow-160-40}Slowdown of checkpoints due to
      interference for the APEX Workshop workflow when simulating the
      Cielo platform with an effective bandwidth of 160GB/s (left) and 40GB/s (right).}
 \end{figure*}
...
interference. Figures~\ref{fig:ckps-slow-160-40}
illustrates how much the average checkpoint
commit is slowed down as a function of the machine MTBF, for two
system I/O bandwidths: 160GB/s, which is the theoretical peak of the
...
(Figure~\ref{fig:ckps-slow-160-40}, left), the Young/Daly formula provides a
...
(Figure~\ref{fig:ckps-slow-160-40}, right), and when the node MTBF is shorter,
