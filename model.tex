...
job $j$ and the checkpoint and recovery durations of job $j$ respectfully.  
We let $\period{Daly}(J_{j}) = \sqrt{2 \ckpt{i} \mu_{i}}$ be the \emph{Daly period}~\cite{young74,daly04} of a job $J_j$ belonging to application class $\app{i}$, where $\mu = \frac{\muind}{\nbnodes{i}}$
and $\muind$ is the MTBF of an individual processor~\cite{springer-monograph}.
At each instance, we schedule as many jobs as possible.  Jobs that are subject to
