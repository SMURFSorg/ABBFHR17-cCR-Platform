...
Jobs inherit these properties from their classes: all jobs
$J_{1 \leq j \leq \nbapp{i}}$ of class $A_{i}$ use $\nbnodes{i}$
nodes.  To simplify notations, we extend the notations to jobs: if $j$
denotes Job $J_j$, we write $\nbnodes{j}, \period{j}, \ckpt{j}$ and
