...
When the system bandwidth is at its peak (Figure~\ref{fig:ckps-slow-160}), the
Young/Daly formula provides a critical tool to minimize interference;
however, when no cooperation between the applications is enforced,
a significant interference is observed, and applications spend in
average twice the time in checkpointing that they did expect. A simple
interference management policy, like FCFS, reduces this dramatically, for all values of the MTBF.  
However, when the bandwidth is more constraint (Figure~\ref{fig:ckps-slow-40}), and when
the node MTBF is shorter, interferences introduce a
significant slowdown for all strategies.
...
storage nodes only to a given application; however, \fifononblock and
