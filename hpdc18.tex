...
To illustrate the second point, we conducted a small simuation (the
parameters and conditions of this simulation are described in more
...
method. Applications either apply the fixed checkpoint period of 1h,
or the optimal period give by the Young/Daly Formula. When two
checkpoints overlap, we can also consider two simple cases: either
...
this can be implemented in Section~\ref{sec:}).
...
also by changing the optimal checkpoint interval), because of
interferences, applications spend more time to commit their checkpoint
...
commit is slowed down as a function of the machine MTBF and for two
system I/O bandwidth (160GB/s, which is the theoretical peak of the
machine, and 40GB/s, which represents a lower bound of the achievable
...
When the system bandwidth is at its peak, one can see that the
...
a significant interference is observed, and applications can spend in
average twice the time they would expect doing checkpoint. A simple
interference management policy, like FCFS, can reduce this in
optimistic scenarios. When the bandwidth is more constraint, or when
the node MTBF is shorter, however, interferences introduce a
