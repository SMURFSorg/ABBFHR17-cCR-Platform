...
\IEEEauthorblockA{Atlanta, GA, USA}
\and
...
platforms, they incur a big resource waste, because they lead to a very
...
\begin{itemize}
  ...
  dramatically decrease the throughput of the platform.
\end{itemize}
...
out of the I/O bandwidth available for checkpoints. We introduce
cooperative algorithms where applications checkpoint in sequence, with a
frequency that is dictated by the priority they dynamically receive from
the scheduler. When there is enough I/O bandwidth available, each
application checkpoints with its optimal Young/Daly period. When it is
