...
operations inherent to the failure-free execution of an application, I/O transfers
from checkpoint/restart (CR) operations are used to ensure progress in the presence
of failures. They place an additional burden, since they increases I/O contention,
leading to degraded performance.  In this work, we consider scheduling
policies and hardware that optimize the
...
performance, thereby maximizing platform throughput. Finally, we consider
how specialized hardware, namely burst buffers, may help to mitigate the I/O
contention problem, and evaluate if they represent an alternative solution
to I/O scheduling.
...
is no I/O interference and minimal re-work to be done when failures
occur.

Last, an alternative approach that has been proposed consists in
adding specialized hardware, the burst buffers, to the computing nodes,
in order to provide an efficient way to overlap I/O operations with
computing operations. In this work, we also consider how the
integration of such components redefines the I/O contention problem.

Altogether, the main contributions of this paper are the following:

\begin{itemize}

\item Development of a model allowing for the quantification of the
  I/O interference of checkpointing applications sharing a common
  underlying I/O substrate.

\item Investigation of the costs of various I/O-aware scheduling
  strategies through both steady-state analysis as well as detailed
  simulations.

\item Investigation of the impact of burst buffers with respect to
  checkpointing strategies for multiple applications sharing the same
  high performance computing infrastructure.

\item A detailed survey of a number scheduling strategies: from
  oblivious algorithms similar to those currently deployed on many
  large-scale platforms, to ones which exploit application knowledge
  in an effort to minimize the total system waste by scheduling the
...
The rest of the paper is organized as follows. Our model is described
in \Cref{sec:model}, followed by a description of the various
scheduling strategies in \Cref{sec:algorithms}. \Cref{sec:lowerbound}
presents a theoretical analysis of the model under a steady-state
scenario, and provides a lower bound of the optimal platform
waste. \Cref{sec:burstbuffers} introduces how we model burst buffers
in this context, and \Cref{sec:simulator} describes the discrete event
simulator used to quantitatively compare the different scheduling
strategies.  \Cref{sec:results} presents the results of the
simulation, providing guidance on the necessary I/O bandwidth for
current and future systems, and assessing the impact of burst buffers on the
overall contention. We conclude with related work
described in \Cref{sec:related}, followed by a summary and future
directions outlined in \Cref{sec:conclusion}.
...
This section extends the framework to the case where each platform
node is equipped with a (private) burst buffer. Multiple architectures
are being considered to integrate these components into
supercomputer nodes, in order to serve as distributed scalable storage mechanisms and
reduce the I/O contention (see~\cite{amm2014}, extended
in~\cite{ammecp2018}, for a taxonomy of envisioned architectures). We
investigate scenarios under which burst buffers help to mitigate I/O
contention when several applications are checkpointing.
...
Transfers from the node burst buffers to the parallel
filesystem can be orchestrated according to some global scheduling
policy, as we did for the case of concurrent direct I/O to the
filesystem. We review below the policies that we used before assuming
the presence of burst buffers, and discuss what changes and what
does not. 
%TODO: don't get the next sentence
Beforehand, we state that the burst buffers of all nodes of
a given application are always processed identically and
simultaneously.

\begin{description}

\item[\nocoop] The strategy becomes non-blocking for the
  applications. Burst-buffers are emptied in parallel when multiple
  files are present in several burst-buffers.
\item[\fifoblock] The strategy becomes non-blocking for the
  applications. Burst-buffers are emptied one application after the
  other.
\item[\fifononblock] The strategy now collapses with \fifoblock. No
  change.
\item[\leastwaste] No change.
\end{description}

In the experiments, we assume that the burst buffers are limited in
...
A different situation can happen with the burst buffer versions of the
different scheduling algorithms: a checkpoint that was only apparently
completed (\emph{i.e.} transferred to the burst buffer, but not yet
entirely to the filesystem) may become concurrent with the application
final output or another checkpoint. As stated above, we prevent two
checkpoints of the same application to compete for the same I/O by
skipping checkpoints that want to start before the previous one is
completed. This may force the application to restart earlier in its
history than it would assume, because the I/O
contentions prevented the use of the desired checkpointing period.
Similarly, we prevent the final output from competing with a
checkpoint, and if a checkpoint transfer from the burst buffer to the
filesystem is not complete by the time the application enters its
final output, the checkpoint transfer is cancelled (risking, in the
case of a failure hitting during the final output to rollback earlier in
the execution).
...
filesystem. Figures~\ref{fig:bb:avsysbw:daly}
and~\ref{fig:bb:avsysbw:fixed} extend the evaluations done above by
...
\begin{figure}
  ...
  \end{center}
  \caption{Waste, with and without burst buffer, as a function of the system bandwidth for the
...
Figures~\ref{fig:bb:mtbf:daly} and~\ref{fig:bb:mtbf:fixed} completes
...
\begin{figure}
  ...
  \end{center}
  \caption{Waste, with and without burst buffer, as a function of the system MTBF for the
    Checkpointing scheduling strategies 
    that checkpoint every 1h, and the LANL workload on
...
execution), the waste drops in the range of 5\% to 10\%.

These evaluation demonstrate the dramatic effect that buffering the
...
configurations of the Cielo platform. Only when at the same time the
available shared filesystem bandwidth is low (under 60GB/s), and the
system is very unreliable (the system MTBF is under 1h), are there
cases where the scheduling strategy can impact significantly the waste
of the system. If the system MTBF is higher or equal 3h, the inclusion
...
Last, we measure the impact of the local bandwidth between the
...
waste of the \propfixed with burst buffers strategy for variable
available bandwidth to the filesystem and the bandwidth to the burst
buffer, and Figure~\ref{fig:bb:bw:daly} considers the waste of the
\propdaly with burst buffers strategy for variable available bandwidth
to the filesystem and to the burst buffer. Interestingly, the figures
...
the factors that dominate the waste are from rollbacks and I/O
that access directly the shared filesystem. As the locality of the burst
buffers architecture permits a perfect scaling, and the number of nodes
...
any reasonable value of the burst buffers bandwidth.
...
\begin{figure}
  \begin{minipage}{0.49\linewidth}
    ...
    \end{center}
    \caption{Waste of the \propfixed with burst buffers scheduling
      strategy as a function of the burst buffer local bandwidth and the system
      available bandwidth to the shared filesystem for the LANL workload on
  ...
  \begin{minipage}{0.49\linewidth}
    ...
    \end{center}
    \caption{Waste of the \propdaly with burst buffers scheduling
      strategy as a function of the burst buffer local bandwidth and the system
      available bandwidth to the shared filesystem for the LANL workload on
...
rollback amount per fault. Such considerations are left for future works.
