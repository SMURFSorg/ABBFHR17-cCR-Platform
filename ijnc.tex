...
the presence of burst buffers, and discuss what would change and what
does not. Beforehand, we state that the burst buffers of all nodes of
...
done (\emph{i.e.} transferred to the burst buffer, but not yet
...
history that it would assume, because of the I/O
contentions. Similarly, we prevent the final output to compete with a
...
case of a failure hitting during the final output to rollback more in
the execution).
...
platform, we assume that each node is equiped with a local burst
buffer, of local bandwidth capacity of 1GB/s, and that is used to
...
\begin{figure}
  ...
  \caption{Waste as a function of the system bandwidth for the
    Checkpointing scheduling strategies with and without burst buffer
    that checkpoint every 1h, and the LANL workload on
    Cielo.\label{fig:bb:avsysbw:1h}}
\end{figure}
...
Both figures show that the inclusion of burst buffers in the system
...
the competiting I/Os performs as well as the best scheduling strategy
(\cooperative, see Figure~\ref{fig:bb:avsysbw:daly}), and outperforms
the best scheduling strategy when using a fixed checkpoint interval
(\fifofixed, see Figure~\ref{fig:bb:avsysbw:fixed}).

Figures~\ref{fig:bb:mtbf:daly} and~\ref{fig:bb:mtbf:fixed} completes
the evaluation by considering a variable system MTBF. As done above,
...
or outperform any other I/O scheduling strategy. The effect of the
...
checkpoints to occupy 80\% to 90\% of the time; with the inclusion of
...
execution), the waste drops to 5\% to 10\%.
...
so significant that if such hardware is available, no particular
...
sufficient to raise the performance that would be achievable by the
...
the factor that dominates the waste is due to rollbacks and to I/O
that access directly the shared filesystem. As the locality of the burst
buffers architecture allows a perfect scaling, and the number of nodes
...
such hardware has a dramatic impact on the performance: even the
simplest strategy becomes more efficient that the most intricate ones
that cannot rely on these buffers, if they become available. Moreover,
...
already completed. There is a trade-off between the platform
utilization and the worst time to completion of an application.
