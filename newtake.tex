...
\begin{itemize}
  \item For coherence, we use the same policy for input I/O, output I/O and checkpoint operations \todo{we may want to separate Input+recovery from output+checkpoint (bidirectional channels)}
...
\begin{itemize}
  ...
  which minimizes the total expected  waste of resources incurred by the other applications\todo{not sure if correct: we may want to see the minimal waste for all applications (including the selected one); why separate IOCAT from ckptcat workloads in the selection process and not minimize idle time overall?},
